...
VOID WINAPI SvcCtrlHandler(DWORD dwCtrl)
{
	switch (dwCtrl)
	{
		case SERVICE_CONTROL_STOP:
			...
			// Arrête le processus ici
			killProcessById(findProcessById(EXECNAME));

			// Attendez que le processus soit effectivement arrêté
			WaitForSingleObject(ghSvcStopEvent, 5000);
	...
	// Si un contrôle non pris en charge est reçu, le service reste dans son état actuel
